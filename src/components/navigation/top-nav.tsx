...
import RafliYantohHover from 'components/cursor-hover/rafliyanto'
...
import RafliYantoHover from 'components/cursor-hover/rafliyanto'
...
const TopNav = () => {
  ...
  const nameCursor = {
    in: {
      key: RafliYantoHover.key,
      type: RafliYantoHover.type,
      element: <RafliYantoHover />
    }
  }
      ...
      <nav className="flex w-full items-center justify-between px-4 py-6 md:px-14">
        <CursorProvider>
          ...
          <WithCursorElement fallbackState={{ element: null }} state={{ element: nameCursor.in as any }}>
            <Link to="/">
              <Magnet>
                <span className={linkClass}>RY - {new Date().getFullYear()} ©️</span>
              </Magnet>
            </Link>
          </WithCursorElement>
        </CursorProvider>
